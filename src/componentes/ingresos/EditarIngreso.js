...
useEffect(()=>{
    const retornarRegistro=async()=>
    {
        ...
        const consulta=await clienteAxios.get(`/ingresoedit/${id}`);
        
        var {usuario,fecha,tipo}=consulta.data
        consulta.data.tipo=tipo
        consulta.data.fecha=moment(fecha).format('LL')
        const diariosConsulta= await clienteAxios.get(`/tipodeingreso/${usuario}`);
    ...
    const  editarIngresos=e=>
    {
        ...
        clienteAxios.put(`/ingresoedit/${ingreso._id}`,ingreso) 
            .then(res => {
