...
function Tabla(props)
{
    ...
    const eliminarRegistro = idTip => {
        Swal.fire({
            ...
            if (result.value) {
                // Llamado a axios
                clienteAxios.delete(`/ingreso/${idTip}`)
                    .then(res => {
                        Swal.fire(  
                            'Eliminado', 
                            res.data.mensaje, 
    ...
    useEffect(()=>{
        ...
        const consultarApi=async()=>{
            try {
                ...
                const diariosConsulta= await clienteAxios.get(`/ingreso/${props.id}`);
                ...
                for(let i=0;i<lista.length;i++)
                {
                    ...
                    var tipoState=await clienteAxios.get(`/tipodeingresoedit/${tipo}`);
        ...
        <tbody>
            ...
            {citas.map(tipGastos=>(
                 <tr>
                        ...
                        <td>
                            <Link to={`/editaringresos/${tipGastos._id}`}>
                                <Icon>
                                    create
                                </Icon>
                            </Link>
